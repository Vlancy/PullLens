...
<h2 id="https">HTTPS with a free certificate</h2>
<p>PullLens ships a second script for this. It is optional, it is never run for you, and the application works without it - but a production instance should not be left on plain HTTP.</p>
<pre><code>./install_ssl.sh</code></pre>
<p>Or without the questions:</p>
<pre><code>./install_ssl.sh pulllens.example.com you@example.com</code></pre>

<p>At the end of <code>./install.sh</code> the same script is offered as a question - <strong>Set up HTTPS now?</strong> - which defaults to <strong>no</strong>. Answer <code>n</code>, or press Enter, and nothing about your installation changes.</p>

<p>What it does, in order:</p>
<ul>
  <li>Moves the PullLens container off port 80 onto an internal port, usually <code>8080</code>, so the proxy can have 80 and 443</li>
  <li>Installs Nginx and certbot with your system package manager - apt, dnf, yum, zypper or pacman</li>
  <li>Writes a reverse proxy for your domain that forwards to the container, websockets included, so the realtime dashboard keeps working over <code>wss://</code></li>
  <li>Requests the certificate with <code>certbot --nginx</code> and turns on the HTTP to HTTPS redirect</li>
  <li>Enables the certbot renewal timer, so the certificate renews itself twice a day without you</li>
  <li>Sets <code>APP_URL</code>, <code>ASSET_URL</code>, <code>SESSION_SECURE_COOKIE</code> and the public websocket settings to the <code>https://</code> address, then rebuilds the stack</li>
</ul>

<div class="warn"><strong>Before you run it</strong>
<p>The script needs a <strong>Linux host</strong> - it stops politely on macOS, because certbot has no system Nginx to configure there - <strong>root or sudo</strong>, a <strong>domain name whose DNS A record already points at the server</strong>, and <strong>ports 80 and 443 free and reachable</strong> from the internet. Let's Encrypt validates by fetching a file over port 80; if that port is closed or the record points elsewhere, issuance fails and the script leaves the instance on HTTP rather than half-configured.</p></div>

<p>It is safe to re-run. A certificate that is still valid is kept, the proxy configuration is rewritten from the same template, and nothing is duplicated. Useful afterwards:</p>
<table>
<tr><th>Command</th><th>What it tells you</th></tr>
<tr><td><code>sudo certbot certificates</code></td><td>Which certificates exist and when they expire</td></tr>
<tr><td><code>sudo certbot renew --dry-run</code></td><td>Proves renewal will work before it matters</td></tr>
<tr><td><code>sudo nginx -t</code></td><td>Checks the proxy configuration at <code>/etc/nginx/sites-available/pulllens.conf</code></td></tr>
</table>

<p>One thing the script cannot do for you: the container still publishes its internal port on every interface, so <code>http://your-server:8080</code> reaches PullLens without TLS. Close that port at the firewall, or at your cloud provider's security group, so visitors can only arrive through 443.</p>

<p>Already have a load balancer, a Cloudflare tunnel or an existing proxy? Skip the script. Point it at the container's port, set <code>APP_URL</code> to the <code>https://</code> address and rerun <code>./install.sh</code> - <code>X-Forwarded-Proto</code> is already trusted.</p>
