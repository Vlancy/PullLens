                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                            Configure encrypted provider credentials and the
                            default model PullLens uses for PR reviews.
                        </p>
                    </div>
                    {!addOpen && (
                        <Button
...
function AddProviderWizard({
    ...
    const driverLabel =
        drivers.find((d) => d.value === data.provider_driver)?.label ??
        data.provider_driver;
            ...
            <ol className="space-y-0">
                ...
                >
                    <div className="grid gap-2 sm:grid-cols-3">
                        {drivers.map((driver) => {
                            const selected =
                                data.provider_driver === driver.value;
                        ...
                        <form onSubmit={submit} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    placeholder={
                                        guide?.baseUrlPlaceholder ||
                                        'Use driver default'
                                    }
                                    aria-invalid={
                                        errors.base_url ? true : undefined
                                    }
                                />
                                {errors.base_url && (
                                    <p className="text-sm text-destructive">
            ...
            <Dialog open={preSaveModalOpen} onOpenChange={setPreSaveModalOpen}>
                <DialogContent className="max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            The provider credentials could not be verified. Fix
                            the issue and try again, or save anyway.
                        </DialogDescription>
                    </DialogHeader>
...
function ProviderCard({
    ...
    const duplicateName = hasDuplicateProviderName(
        data.name,
        providers,
        provider.id,
    );
                ...
                {editOpen && (
                    <CardContent className="border-t pt-5">
                        <form onSubmit={update} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor={`${formId}-base-url`}>
                                    {provider.provider_driver === 'bedrock'
                                        ? 'AWS region'
                                        : 'Base URL'}{' '}
                                    <span className="font-normal text-muted-foreground">
                                        (optional)
...
function hasDuplicateProviderName(
    ...
    return providers.some(
        (p) =>
            p.id !== exceptId &&
            p.name.trim().toLocaleLowerCase() === normalized,
    );
}
